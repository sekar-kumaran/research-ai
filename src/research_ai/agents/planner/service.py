...
16. simple_answer
    Purpose : Answer simple educational/non-literature questions without paper retrieval.
    Args    : {"query": "<string>"}
    When    : User asks a basic definition or explanation and does not ask for papers/latest evidence.
"""
...
_OLLAMA_SYSTEM_PROMPT = (
    ...
    "Tools: hybrid_search, metadata_rag, classify_query, summarize, methodology_extract, simple_answer.\n"
...
class PlannerAgent:
    ...
    def plan(
        ...
        if mode_hint == "auto" and self._is_simple_answer(query):
            q = (query or "").strip()
            return ResearchPlan(
                intent="simple_answer",
                query=q,
                top_k=self._clamp(top_k),
                calls=[ToolCall("simple_answer", {"query": q})],
                reason="simple_question_no_retrieval_needed",
                used_fallback=True,
            )
        ...
        max_tok = 800 if is_ollama else 1000
        
        for attempt in range(2):
            try:
                raw = cloud.generate(prompt, max_tokens=max_tok, system=sys, json_mode=True)
    ...
    @staticmethod
    def _is_simple_answer(query: str) -> bool:
        q = " ".join((query or "").strip().lower().split())
        research_terms = (
            "latest", "recent", "paper", "papers", "literature", "survey",
            "study", "studies", "evidence", "citation", "compare", "approaches",
            "state of the art", "sota",
        )
        simple_prefixes = ("what is ", "what are ", "define ", "explain ")
        return q.startswith(simple_prefixes) and not any(term in q for term in research_terms) and len(q.split()) <= 12
    ...
    def _fallback_plan(
        ...
        if (
            mode == "auto"
            and q.lower().startswith(simple_prefixes)
            and not any(term in lower for term in research_terms)
            and len(q.split()) <= 12
        ):
            return ResearchPlan(
                intent="simple_answer",
                query=q, top_k=k,
                calls=[ToolCall("simple_answer", {"query": q})],
                reason="heuristic_simple_answer",
                used_fallback=True,
            )
