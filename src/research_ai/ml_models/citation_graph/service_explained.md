...
Generated educational companion for `src/research_ai/ml_models/citation_graph/service.py`. This file is intentionally detailed so a developer can understand the code, architecture role, production tradeoffs, and ML/backend concepts behind the implementation.

## File Overview

`src/research_ai/ml_models/citation_graph/service.py` is a Python module in the ML services layer: classifiers, summarizers, similarity, ranking, and extraction. It defines CitationGraphService and no top-level functions.
...
**Current step:** `src/research_ai/ml_models/citation_graph/service.py` performs its local responsibility.
...
flowchart LR
  ...
  API --> ThisFile[src/research_ai/ml_models/citation_graph/service.py]
...
- **Line:** 4
- **Base classes:** `object`
- **Docstring:** Citation intelligence facade.
...
**Methods:**
- `related_signals` at line 12: method behavior is described by its body and name
...
### Class `CitationGraphService` Methods

#### `CitationGraphService.related_signals`
...
- `src/research_ai/ml_models/citation_graph/service.py` is connected through imports, startup scripts, API routes, frontend selectors, tests, or artifact paths.
...
- `src/research_ai/ml_models/citation_graph/service.py` should be understood as part of a layered AI research platform.
...
# L0009: Executes Python logic as part of this file's workflow; read surrounding lines for data flow and side effects.
    signals from category/year/title metadata until a citation graph is trained.
# L0010: Starts, ends, or continues a docstring that documents module, class, or function intent.
    """
# L0011: Blank line that visually separates logical sections and improves readability.

# L0012: Defines a function or method; parameters are the input contract and the body implements the workflow.
    def related_signals(self, papers: list[dict]) -> dict:
...
# L0014: Assigns or updates a value used later in the workflow; check mutability and data shape.
        years: dict[str, int] = {}
# L0015: Iterates over data, retry attempts, files, results, or workflow steps.
        for paper in papers:
# L0016: Iterates over data, retry attempts, files, results, or workflow steps.
            for category in str(paper.get("category", "")).split():
