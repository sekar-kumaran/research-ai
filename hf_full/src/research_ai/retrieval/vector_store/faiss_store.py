...
class FaissVectorStore:
    ...
    def search(self, query_vec: np.ndarray, top_k: int) -> list[RetrievedDocument]:
        ...
        for score, idx in zip(scores[0], ids[0]):
            ...
            # Metadata columns vary by index build — map gracefully.
            # Actual columns in current index: id, title, abstract, broad_category, cluster_id
            # Fallback chain: categories -> broad_category -> category -> ""
            category = (
                str(row.get("categories") or row.get("broad_category") or row.get("category") or "")
            )
            # Year: prefer update_date prefix, else try to extract from paper_id (arXiv IDs
            # often start with YYMM.xxxxx, e.g. 2301.12345 → year "2023")
            raw_date = str(row.get("update_date") or row.get("published_date") or "")
            if raw_date:
                year = raw_date[:4]
            else:
                pid = str(row.get("id", ""))
                # arXiv new-style: YYMM.nnnnn — first 2 digits are year offset from 2000
                year = ""
                if len(pid) >= 4 and pid[:2].isdigit():
                    prefix = int(pid[:2])
                    year = str(2000 + prefix) if prefix < 100 else ""
